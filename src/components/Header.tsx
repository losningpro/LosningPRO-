...
const Header = () => {
  ...
  const onSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const q = query.trim();
    navigate(q ? `/kob?q=${encodeURIComponent(q)}` : "/kob");
          ...
          <div className="flex items-center space-x-3 flex-shrink-0">
            <Link to="/" className="flex items-center space-x-2">
              ...
              <span className="text-2xl font-bold leading-none">
                <span className="text-black">Løsning</span>
            ...
            <nav className="flex items-center justify-center space-x-6">
              <Link
                ...
                className={`text-sm font-medium transition-colors hover:text-blue-600 ${
                  isActive("/kob") || isActive("/køb") ? "text-blue-600" : "text-blue-700"
                }`}
              >
                Køb
              </Link>
              ...
              <Link
                to="/tjenester"
                className={`text-sm font-medium transition-colors hover:text-blue-600 ${
                  isActive("/tjenester") || location.pathname.startsWith("/tjenester/")
                    ? "text-blue-600"
                    : "text-blue-700"
                }`}
              >
              ...
              <Link
                to="/hvordan-fungerer"
                className={`text-sm font-medium transition-colors hover:text-blue-600 ${
                  isActive("/hvordan-fungerer") ? "text-blue-600" : "text-blue-700"
                }`}
              >
                Hvordan fungerer
              </Link>
              ...
              <Link
                to="/om-os"
                className={`text-sm font-medium transition-colors hover:text-blue-600 ${
                  isActive("/om-os") ? "text-blue-600" : "text-blue-700"
                }`}
              >
                Om os
              </Link>
              ...
              <Link
                to="/arbejdsgalleri"
                className={`text-sm font-medium transition-colors hover:text-blue-600 ${
                  isActive("/arbejdsgalleri") ? "text-blue-600" : "text-blue-700"
                }`}
              >
                Arbejdsgalleri
              </Link>
              ...
              <Link
                to="/kontakt"
                className={`text-sm font-medium transition-colors hover:text-blue-600 ${
                  isActive("/kontakt") ? "text-blue-600" : "text-blue-700"
                }`}
              >
                Kontakt
              </Link>
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden border-t border-gray-200 py-4">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              ...
              >
                <Phone className="h-4 w-4" />
                <span className="font-medium">Ring nu: +45 52 71 78 10</span>
              </a>
            </div>
          </div>
        )}
